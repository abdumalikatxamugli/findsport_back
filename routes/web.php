...
Route::middleware(['auth'])->group(function () {
    Route::prefix('admin')->group(function(){
        ...
        Route::match(['get', 'post'], '/places/create', "Admin\Places@create")
                    ->name('places.create');
        Route::get('place/delete/{id}', "Admin\Places@delete")
                    ->name("places.delete");
        Route::match(['get', 'post'], '/place/{id}/', "Admin\Places@one")
                    ->name('places.edit');

        // sections
        Route::get('/sections', "Admin\Section@list")->name("sections.list");

        Route::match(['get', 'post'], '/sections/create', "Admin\Section@create")
                    ->name('sections.create');
        Route::get('section/delete/{id}', "Admin\Section@delete")
                    ->name("sections.delete");
        Route::match(['get', 'post'], '/section/{id}/', "Admin\Section@one")
                    ->name('sections.edit');
        ...
        Route::get("/messages", "MessageController@list")->name("messages_list");
   });
});
...
// Create message
Route::post('/message_create','MessageController@create')->name('create_message');
