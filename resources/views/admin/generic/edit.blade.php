					...
					<?foreach ($fields as $name => $value): ?>
						<?if($value['file']??false):?>
							<div>
								...
								<?if($post->path??false):?>
								<img src="{{route('unimg')."?path=".urlencode($post->path)}}" alt="">
									<a href="{{route($entity.'_delete_img',$post->id)}}">
										remove image
									</a>
								<?endif?>
							</div>
							...
							<?foreach($value['custom_json_fields'] as $prop):?>
								...
								<input type="{{$value['type']}}" 
									...
									<?=
										...
										:null
									?>
									value="{{json_decode($post->$name,true)[$prop]??null}}"
								><br>
							<?endforeach?>
							</div>
						<?endif?>
...
<div id="tabs" style="display: none;">
	<ul class="nav nav-tabs" id="myTab" role="tablist">
		...
		<li class="nav-item">
			<a class="nav-link <?=$index==0?'active':''?> {{$lang}}nav" 
				id="{{$lang}}-tab" 
				data-toggle="tab" href="#{{$lang}}" 
	...
	<div class="tab-content">
		<?foreach($langs as $index=>$lang):?>
		<div class="tab-pane fade show {{$lang}} <?=$index==0?'active':''?>" 
